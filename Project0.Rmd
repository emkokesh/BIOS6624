...
# extract fixed effects from the random intercept model
q1_final_table <- tidy(model_intercept, effects = "fixed", conf.int = TRUE) %>%
  mutate(
    term = case_when(
      term == "(Intercept)" ~ "Systemic Bias (Minutes)",
      term == "MEMs_Min_Since_Wake" ~ "Agreement Slope",
      TRUE ~ term
    ),
    estimate = round(estimate, 3),
    std.error = round(std.error, 3), # rounded to 3 decimal places
...
q1_final_table %>%
  ...
  kable(
    ...
    caption = "Agreement between booklet and cap",
    align = "llccc"
  )
