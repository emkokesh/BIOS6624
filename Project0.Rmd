...
agreement_model_slope <- lmer(Booklet_Min_Since_Wake ~ MEMs_centered + 
                                (1 + MEMs_centered | SubjectID), 
                              data = df_q1,
                              control = lmerControl(optimizer = "nlminbwrap",
                                                   optCtrl = list(maxfun = 1e5)))
                              
summary(agreement_model_slope)

#compare to see if the random slope improves fit
anova(model_intercept, agreement_model_slope)
...
df_cort_clean <- df %>%
  filter(!is.na(MEMs_Min_Since_Wake)) %>%
  filter(!is.na(Cortisol_nmol)) %>%
  filter(Cortisol_nmol <= 80) %>%
  mutate(
    log_cort = log(Cortisol_nmol)
  )
...
df_dhea_clean <- df %>%
  ...
  mutate(
    ...
    DHEA_nmol = ifelse(!is.na(DHEA_nmol) & DHEA_nmol >= 5.205, NA, DHEA_nmol),
...
#naive linear models (assumes independence)
cort_lm <- lm(log_cort ~ time_rise + time_decline, data = df_cort_clean)
dhea_lm <- lm(log_DHEA ~ time_rise + time_decline, data = df_dhea_clean)

#piecewise linear mixed models

#cortisol
cort_model <- lmer(log_cort ~ time_rise + time_decline + (1 | SubjectID), 
                   data = df_cort_clean,
                   REML = FALSE)
summary(cort_model)

#DHEA model 
dhea_model <- lmer(log_DHEA ~ time_rise + time_decline + (1 | SubjectID), 
                   data = df_dhea_clean,
                   REML = FALSE)
summary(dhea_model)

#compare model fit to justify random intercept
AIC(cort_lm, cort_model)
AIC(dhea_lm, dhea_model)

#calculate icc to quantify within-subject clustering
get_icc <- function(model) {
  vc <- as.data.frame(VarCorr(model))
  var_subject <- vc$vcov[vc$grp == "SubjectID"]
  var_resid <- attr(VarCorr(model), "sc")^2
  var_subject / (var_subject + var_resid)
}

icc_cort <- get_icc(cort_model)
icc_dhea <- get_icc(dhea_model)

print(paste("icc cortisol:", round(icc_cort, 3)))
print(paste("icc dhea:", round(icc_dhea, 3)))

#check residual diagnostics on log scale
par(mfrow = c(1, 2))
plot(fitted(cort_model), resid(cort_model),
     xlab = "fitted values", ylab = "residuals",
     main = "cortisol lmm residuals vs fitted")
abline(h = 0, lty = 2)
qqnorm(resid(cort_model), main = "cortisol lmm qq plot")
qqline(resid(cort_model))
par(mfrow = c(1, 1))

par(mfrow = c(1, 2))
plot(fitted(dhea_model), resid(dhea_model),
     xlab = "fitted values", ylab = "residuals",
     main = "dhea lmm residuals vs fitted")
abline(h = 0, lty = 2)
qqnorm(resid(dhea_model), main = "dhea lmm qq plot")
qqline(resid(dhea_model))
par(mfrow = c(1, 1))
...
hormone_final_table <- bind_rows(cort_tidy_ci, dhea_tidy_ci) %>%
  mutate(
    ...
    #added: correct back-transforms to match q3 questions
    estimate_bt = case_when(
      term == "Wake Level (Intercept)" ~ exp(estimate),
      term == "Morning Change (0-30 min)" ~ exp(estimate),
      term == "Daily Decline (30+ min)" ~ exp(estimate),
      TRUE ~ exp(estimate)
    ),
    conf_low_bt = case_when(
      term == "Wake Level (Intercept)" ~ exp(conf.low),
      term == "Morning Change (0-30 min)" ~ exp(conf.low),
      term == "Daily Decline (30+ min)" ~ exp(conf.low),
      TRUE ~ exp(conf.low)
    ),
    conf_high_bt = case_when(
      term == "Wake Level (Intercept)" ~ exp(conf.high),
      term == "Morning Change (0-30 min)" ~ exp(conf.high),
      term == "Daily Decline (30+ min)" ~ exp(conf.high),
      TRUE ~ exp(conf.high)
    ),
    estimate_bt = round(estimate_bt, 4),
    conf_interval_bt = paste0("[", round(conf_low_bt, 4), ", ", round(conf_high_bt, 4), "]"),
    p.value = ifelse(p.value < 0.001, "< 0.001", round(p.value, 3))
  ) %>%
  select(Hormone, term, estimate_bt, conf_interval_bt, p.value)


#make table 
hormone_final_table %>%
  kable(
    col.names = c("Hormone", "Model Parameter", "Back-Transformed Estimate", "95% CI (Back-Transformed)", "p-value"),
    caption = "Diurnal Hormone Trajectories With Back-Transformed Estimates and 95% Confidence Intervals",
    align = "llccc"
  )
...
ggplot(df_cort_clean, aes(x = MEMs_Min_Since_Wake, y = log_cort)) +
  ...
  labs(
    title = "MEMs Minutes Since Wake vs Log Cortisol (nmol/L)",
    x = "MEMs Minutes Since Wake",
    y = "Log Cortisol (nmol/L)"
  )

#dhea plot with overlay
ggplot(df_dhea_clean, aes(x = MEMs_Min_Since_Wake, y = log_DHEA)) +
  geom_point(alpha = 0.3) +
  ...
  labs(
    title = "MEMs Minutes Since Wake vs Log DHEA (nmol/L)",
    x = "MEMs Minutes Since Wake",
    y = "Log DHEA (nmol/L)"
  )


#mean and ci by sample figure
cort_by_sample <- df_cort_clean %>%
  group_by(Collection.Sample) %>%
  summarise(
    mean = mean(Cortisol_nmol, na.rm = TRUE),
    sd = sd(Cortisol_nmol, na.rm = TRUE),
    n = sum(!is.na(Cortisol_nmol)),
    se = sd / sqrt(n),
    ci_low = mean - 1.96 * se,
  ...
  mutate(
    Hormone = "Cortisol",
    sample_point = case_when(
      Collection.Sample == 1 ~ "S1",
      Collection.Sample == 2 ~ "S2",
      Collection.Sample == 3 ~ "S3",
      Collection.Sample == 4 ~ "S4"
    )
  )

dhea_by_sample <- df_dhea_clean %>%
  group_by(Collection.Sample) %>%
  summarise(
    mean = mean(DHEA_nmol, na.rm = TRUE),
    sd = sd(DHEA_nmol, na.rm = TRUE),
    n = sum(!is.na(DHEA_nmol)),
    se = sd / sqrt(n),
    ci_low = mean - 1.96 * se,
    ci_high = mean + 1.96 * se,
    .groups = "drop"
...
mean_ci_plot_df <- bind_rows(cort_by_sample, dhea_by_sample) %>%
  mutate(sample_point = factor(sample_point, levels = c("S1", "S2", "S3", "S4")))

ggplot(mean_ci_plot_df, aes(x = sample_point, y = mean)) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = ci_low, ymax = ci_high), width = 0.15) +
  facet_wrap(~Hormone, scales = "free_y") +
  presentation_theme +
  labs(
    title = "Mean Hormone Levels by Sample Point",
    subtitle = "Mean ± 95% CI (nmol/L)",
    x = "Sample Point",
    y = "Mean (nmol/L)"
  )
