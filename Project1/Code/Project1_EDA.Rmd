...
table2_outcomes <- df_a %>%
  filter(years == 2) %>%
  select(hard_drugs_bl, vload, leu3n, agg_phys, agg_ment, adh_cat) %>%
  tbl_summary(
    by = hard_drugs_bl,
    label = list(
      vload ~ "Year 2 Viral Load",
      leu3n ~ "Year 2 CD4+ Count",
      adh_cat ~ "Year 2 Adherence Score"
    )
  ) %>%
  add_p() %>%
...
Section 4: Initial exploration of outcome distributions
```{r}
...
ggplot(df_y2, aes(x = log10(vload))) + geom_histogram(bins = 60) + labs(title = "Year 2: log10(VLOAD+1)")
