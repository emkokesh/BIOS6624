---
title: "Project1_EDA.Rmd"
output: html_document
---

Section 0: Load Data
```{r}
library(tidyverse)
library(janitor)
library(skimr)
library(broom)
library(gtsummary)

path_raw <- "C:/Users/kokeshem/Spring 2026/Advanced Statistical Methods/Project 1/hiv_6624_final.csv"

#load data
df <- read.csv(path_raw, na.strings = c("", "NA")) %>%
  clean_names()

#inspect data
names(df)
``` 

Section 1: Data and Structure Checks, Missingness, create analytic Dataset

```{r}
#drop index column
df <- df %>% select(-any_of("x"))
glimpse(df)

df %>% summarise(n_subjects_total = n_distinct(newid))

#quick structure check- number of unique individuals per year
df %>% summarise(
  n_rows = n(),
  n_subjects = n_distinct(newid),
  min_year = min(years, na.rm = TRUE),
  max_year = max(years, na.rm = TRUE)
)

df %>% count(years) %>% arrange(years)

#visits per subject
visits_by_id <- df %>% count(newid, name = "n_visits")
summary(visits_by_id$n_visits)

#quick visit distribution summary
df %>%
  count(newid, name = "n_visits") %>%
  summarise(
    min_visits = min(n_visits),
    p25_visits = quantile(n_visits, 0.25),
    median_visits = median(n_visits),
    p75_visits = quantile(n_visits, 0.75),
    max_visits = max(n_visits)
  )

#check that art is 0 at year 0 and 1 at every other year
df %>%
  group_by(years) %>%
  summarise(
    n = n(),
    prop_art_1 = mean(art == 1, na.rm = TRUE),
    prop_art_missing = mean(is.na(art))
  ) %>%
  arrange(years)

#create a subject level baseline (year 0) hard drug use indicator and merge it onto all follow up rows
baseline_exposure <- df %>%
  filter(years == 0) %>%
  select(newid, hard_drugs) %>%
  rename(hard_drugs_bl = hard_drugs)

df2 <- df %>% left_join(baseline_exposure, by = "newid")

#visit availability

visit_availability <- df2 %>%
  group_by(newid) %>%
  summarise(
    have_y0 = any(years == 0),
    have_y2 = any(years == 2),
    .groups = "drop"
  ) %>%
  summarise(
    n_have_y0   = sum(have_y0),
    n_have_y2   = sum(have_y2),
    n_have_both = sum(have_y0 & have_y2)
  )

print(visit_availability)



df2 %>%
  distinct(newid, hard_drugs_bl) %>%
  count(hard_drugs_bl, name = "n_subjects")

df2 %>%
  summarise(
    n_subjects = n_distinct(newid),
    prop_missing_hard_drugs_bl = mean(is.na(hard_drugs_bl))
  )


#copy of raw bmi for auditing
df2 <- df2 %>%
  mutate(bmi_raw = bmi)

#flag rows that will be recoded
bmi_flagged_pre <- df2 %>%
  transmute(
    newid, years,
    bmi_raw,
    flag_reason = case_when(
      bmi_raw %in% c(-1, 999) ~ "coded missing (-1/999)",
      !is.na(bmi_raw) & bmi_raw > 80 ~ "bmi > 80",
      TRUE ~ NA_character_
    )
  ) %>%
  filter(!is.na(flag_reason)) %>%
  arrange(flag_reason, desc(bmi_raw), newid, years)

#summarize how many bmi values are affected
bmi_audit_pre <- bmi_flagged_pre %>%
  summarise(
    n_rows_flagged = n(),
    n_ids_flagged  = n_distinct(newid),
    n_rows_missing_code = sum(flag_reason == "coded missing (-1/999)"),
    n_rows_gt80         = sum(flag_reason == "bmi > 80"),
    n_ids_missing_code  = n_distinct(newid[flag_reason == "coded missing (-1/999)"]),
    n_ids_gt80          = n_distinct(newid[flag_reason == "bmi > 80"])
  )

print(bmi_audit_pre)

#show the affected rows
print(bmi_flagged_pre)

#apply bmi cleaning
df2 <- df2 %>%
  mutate(
    bmi = na_if(bmi, 999),
    bmi = na_if(bmi, -1),
    bmi = ifelse(!is.na(bmi) & bmi > 80, NA, bmi)
  )

#confirm bmi is NA for all flagged rows after cleaning
bmi_check_post <- df2 %>%
  filter(newid %in% bmi_flagged_pre$newid, years %in% bmi_flagged_pre$years) %>%
  left_join(bmi_flagged_pre, by = c("newid", "years")) %>%
  summarise(
    n_flagged_rows = n(),
    n_still_nonmissing = sum(!is.na(bmi)),
    .groups = "drop"
  )

print(bmi_check_post)

#merge the cleaned bmi back onto the exact flagged (newid, years) rows
bmi_flagged_post <- bmi_flagged_pre %>%
  left_join(df2 %>% select(newid, years, bmi), by = c("newid", "years")) %>%
  arrange(flag_reason, desc(bmi_raw), newid, years)

#summary check
bmi_check_post_fixed <- bmi_flagged_post %>%
  summarise(
    n_flagged_rows = n(),
    n_still_nonmissing = sum(!is.na(bmi)),
    .groups = "drop"
  )

print(bmi_check_post_fixed)

# if anything is unexpectedly still non-missing, show it
bmi_still_nonmissing <- bmi_flagged_post %>%
  filter(!is.na(bmi))

print(bmi_still_nonmissing)




#create analysis dataset with only required covariates
analysis_vars <- c(
  "newid", "years",
  "hard_drugs_bl",
  "vload", "leu3n", "agg_phys", "agg_ment",
  "age", "bmi", "educbas", "race", "smoke", "adh"
)
df_a0 <- df2 %>%
  select(all_of(analysis_vars))

#merge levels of race and education
unique(df_a0$race)

df_a1 <- df_a0 %>%
  mutate(
    race_cat = case_when(
      race == 1              ~ "White (NH)",
      race %in% c(2, 3, 4, 7, 8) ~ "Other",
      TRUE ~ NA_character_
    ),
    educ_cat = case_when(
      educbas %in% c(1, 2, 3, 4) ~ "No College Degree",
      educbas %in% c(5, 6, 7) ~ "College+",
      TRUE ~ NA_character_
    ),
    smoke_cat = case_when(
      smoke %in% c(1,2) ~ "Non-Current Smoker",
      smoke == 3 ~ "Current Smoker", 
      TRUE ~ NA_character_
    ),
    adh_cat = case_when(
      adh %in% c(1,2) ~ "At least 95%",
      adh %in% c(3,4) ~ "Below 95%",
      TRUE ~ NA_character_
    )
  )

#factor categorical variables
df_a <- df_a1 %>%
  mutate(
    race_cat = factor(race_cat, levels = c("White (NH)", "Other")),
    educ_cat = factor(educ_cat, levels = c("No College Degree", "College+")),
    smoke_cat = factor(smoke_cat, levels= c("Non-Current Smoker", "Current Smoker")),
    adh_cat= factor(adh_cat, levels=c("At least 95%", "Below 95%")),
    hard_drugs_bl = factor(hard_drugs_bl)
  )

#convert hard_drugs_bl labels to yes/no
df_a <- df_a %>%
  mutate(
    hard_drugs_bl = fct_recode(hard_drugs_bl, "No" = "0", "Yes" = "1")
  )

#check the new counts at baseline
df_a %>% filter(years == 0) %>% count(race_cat, sort = TRUE)
df_a %>% filter(years == 0) %>% count(educ_cat, sort = TRUE)

#check smoke at baseline
df_a %>% filter(years == 0) %>% count(smoke, sort = TRUE)

#check baseline sample size and group balance
df_bl <- df_a %>% filter(years == 0)

#how many subjects total and per group
df_bl %>%
  distinct(newid, hard_drugs_bl) %>%
  count(hard_drugs_bl)

#check baseline covariate distributions by exposure
df_bl %>%
  group_by(hard_drugs_bl) %>%
  summarise(
    n = n_distinct(newid),
    age_mean = mean(age, na.rm = TRUE),
    bmi_mean = mean(bmi, na.rm = TRUE)
  )

df_bl %>% count(hard_drugs_bl)

#check baseline sample size and group balance
df_y2 <- df_a %>% filter(years == 2)


#how many subjects total and per group
df_y2 %>%
  distinct(newid, hard_drugs_bl) %>%
  count(hard_drugs_bl)

#check baseline covariate distributions by exposure
df_y2 %>%
  group_by(hard_drugs_bl) %>%
  summarise(
    n = n_distinct(newid),
    age_mean = mean(age, na.rm = TRUE),
    bmi_mean = mean(bmi, na.rm = TRUE)
  )

df_y2 %>% count(hard_drugs_bl)


#check year 2 availability (dropout) by baseline hard drug group
have_y2 <- df_a %>%
  group_by(newid) %>%
  summarise(
    have_year2 = any(years == 2),
    hard_drugs_bl = first(hard_drugs_bl)
  )

have_y2 %>% count(have_year2)
have_y2 %>% count(hard_drugs_bl, have_year2)

have_y2 %>%
  group_by(hard_drugs_bl) %>%
  summarise(
    n = n(),
    prop_have_y2 = mean(have_year2)
  )

#missingness at year 2 for each outcome (by baseline hard drug group)
df_y2 <- df_a %>% filter(years == 2)

#overall year 2 adherence missingness (all subjects with a year 2 visit)
df_y2 %>%
  summarise(
    n_rows = n(),
    n_subjects = n_distinct(newid),
    prop_missing_adh = mean(is.na(adh_cat))
  )

#year 2 adherence missingness by baseline hard drug group
df_y2 %>%
  group_by(hard_drugs_bl) %>%
  summarise(
    n_subjects = n_distinct(newid),
    miss_vload = mean(is.na(vload)),
    miss_leu3n = mean(is.na(leu3n)),
    miss_agg_phys = mean(is.na(agg_phys)),
    miss_agg_ment = mean(is.na(agg_ment)),
    miss_adh = mean(is.na(adh_cat))
  )

#year 2 missingness for key covariates
df_y2 %>%
  group_by(hard_drugs_bl) %>%
  summarise(
    miss_age = mean(is.na(age)),
    miss_bmi = mean(is.na(bmi)),
    miss_race = mean(is.na(race_cat)),
    miss_educ = mean(is.na(educ_cat)),
    miss_smoke = mean(is.na(smoke_cat))
  )

#baseline covariate missingness (exclude adh because it is not measured at year 0)
df_bl %>%
  summarise(across(c(age, bmi, race_cat, educ_cat, smoke_cat, hard_drugs_bl),
                   ~ mean(is.na(.)))) %>%
  pivot_longer(everything(), names_to="var", values_to="prop_missing") %>%
  arrange(desc(prop_missing))

#restrict analytic sample to subjects that have all covariate and outcome data at year 0 and year 2

df_a_pre_sample <- df_a 

outcomes <- c("vload", "leu3n", "agg_phys", "agg_ment")

# baseline covariates used across all models
covars_y0 <- c("hard_drugs_bl", "age", "bmi", "race_cat", "educ_cat", "smoke_cat")

complete_flags <- df_a_pre_sample %>%
  filter(years %in% c(0, 2)) %>%
  group_by(newid) %>%
  summarise(
    have_y0 = any(years == 0),
    have_y2 = any(years == 2),

    #outcomes complete at Y0 and Y2
    y0_outcomes_complete = any(years == 0 & if_all(all_of(outcomes), ~ !is.na(.x))),
    y2_outcomes_complete = any(years == 2 & if_all(all_of(outcomes), ~ !is.na(.x))),

    #baseline covariates complete at Y0
    y0_covars_complete = any(years == 0 & if_all(all_of(covars_y0), ~ !is.na(.x))),

    #adherence complete at Y2 only
    y2_adh_complete = any(years == 2 & !is.na(adh_cat)),

    #year 2 covariates complete at Y2
    y2_covars_complete = any(years == 2 & if_all(all_of(covars_y0), ~ !is.na(.x))),

    .groups = "drop"
  ) %>%
  mutate(
    keep_subject = have_y0 & have_y2 &
      y0_outcomes_complete & y2_outcomes_complete &
      y0_covars_complete & y2_adh_complete
  )

#complete-case denominator and kept samples
complete_case_counts <- complete_flags %>%
  summarise(
    n_evaluated = n(),
    n_kept      = sum(keep_subject),
    n_excluded  = sum(!keep_subject)
  )

print(complete_case_counts)

ids_keep <- complete_flags %>% filter(keep_subject) %>% select(newid)

#apply restriciton
df_a <- df_a_pre_sample %>% semi_join(ids_keep, by = "newid") %>% filter(years %in% c(0,2))

#who was removed
removed_summary <- complete_flags %>%
  filter(!keep_subject) %>%
  mutate(
    reason = case_when(
      !have_y0 ~ "No Year 0 visit",
      !have_y2 ~ "No Year 2 visit",
      have_y0 & have_y2 & !y0_outcomes_complete ~ "Missing 1+ outcome at Year 0",
      have_y0 & have_y2 & !y2_outcomes_complete ~ "Missing 1+ outcome at Year 2",
      have_y0 & have_y2 & y0_outcomes_complete & y2_outcomes_complete & !y0_covars_complete ~ "Missing 1+ baseline covariate (Year 0)",
      have_y0 & have_y2 & y0_outcomes_complete & y2_outcomes_complete & y0_covars_complete & !y2_adh_complete ~ "Missing adherence at Year 2",
      TRUE ~ "Other"
    )
  )

#counts of removed subjects by reason
removed_summary %>% count(reason, sort = TRUE)


#key Y0/Y2 rows for removed subjects
removed_rows <- df_a_pre_sample %>%
  semi_join(removed_summary %>% select(newid), by = "newid") %>%
  filter(years %in% c(0, 2)) %>%
  select(newid, years, all_of(outcomes), all_of(covars_y0), adh_cat) %>%
  arrange(newid, years)

removed_rows

#final sample size (subjects)
df_a %>% summarise(n_subjects = n_distinct(newid))
```







#Section 2: Check outliers/impossible values for all covariates and outcomes, create final analytical dataset (df_a)

```{r}
#check outliers/impossible values for all covariates and outcomes

#restrict to years 0 and 2
df_02 <- df_a %>%
  filter(years %in% c(0, 2)) %>%
  mutate(
    #create flags for potential issues
    flag_vload_neg = !is.na(vload) & vload < 0,
    flag_vload_hi  = !is.na(vload) & vload > 1000000,     #>1,000,000 copies/mL (rare but possible)

    flag_cd4_nonpos = !is.na(leu3n) & leu3n <= 0,
    flag_cd4_hi     = !is.na(leu3n) & leu3n > 3000,        #very high; flag for review

    flag_bmi_low = !is.na(bmi) & bmi < 10,                 #extreme low
    flag_bmi_hi  = !is.na(bmi) & bmi > 80,                 #extreme high

    flag_age_low = !is.na(age) & age < 18,                 #cohort is adult men, flag <18
    flag_age_hi  = !is.na(age) & age > 100,                #extreme high

    flag_phys_out = !is.na(agg_phys) & (agg_phys < 0 | agg_phys > 100),
    flag_ment_out = !is.na(agg_ment) & (agg_ment < 0 | agg_ment > 100),

    # any issue on this row?
    flag_any_issue = flag_vload_neg | flag_vload_hi |
      flag_cd4_nonpos | flag_cd4_hi |
      flag_bmi_low | flag_bmi_hi |
      flag_age_low | flag_age_hi |
      flag_phys_out | flag_ment_out
  )

#one table of all observations with any flag + what the problem is
issues_tbl <- df_02 %>%
  filter(flag_any_issue) %>%
  mutate(
    issue = case_when(
      flag_vload_neg ~ "vload < 0",
      flag_vload_hi  ~ "vload > 1,000,000",
      flag_cd4_nonpos ~ "leu3n <= 0",
      flag_cd4_hi     ~ "leu3n > 3000",
      flag_bmi_low    ~ "bmi < 10",
      flag_bmi_hi     ~ "bmi > 80",
      flag_age_low    ~ "age < 18",
      flag_age_hi     ~ "age > 100",
      flag_phys_out   ~ "agg_phys outside 0-100",
      flag_ment_out   ~ "agg_ment outside 0-100",
      TRUE ~ NA_character_
    )
  ) %>%
  select(
    newid, years,
    vload, leu3n, bmi, age, agg_phys, agg_ment,
    issue
  ) %>%
  arrange(newid, years)

issues_tbl


#data that has been cleaned will be saved to DataProcessed folder
project1_clean <- df_a
out_dir <- "C:/Users/kokeshem/Spring 2026/Advanced Data Analysis/BIOS6624/Project1/DataProcessed"
if (!dir.exists(out_dir)) dir.create(out_dir, recursive = TRUE)

# write file
write.csv(project1_clean, file.path(out_dir, "project1_clean.csv"), row.names = FALSE)



```

Section 3: Table 1

```{r}

#filter for baseline
table1_data <- df_a %>% 
  filter(years == 0) %>%
  distinct(newid, .keep_all = TRUE) %>%
  #select the variables that describe the subjects at baseline
  select(hard_drugs_bl, age, bmi, race_cat, educ_cat, smoke_cat, vload, leu3n, agg_phys, agg_ment)

#create the table
table1 <- table1_data %>%
  tbl_summary(
    by = hard_drugs_bl,
    label = list(
      age ~ "Age (years)",
      bmi ~ "BMI (kg/m^2)",
      race_cat ~ "Race/Ethnicity",
      educ_cat ~ "Education Level",
      smoke_cat ~ "Smoking Status",
      vload ~ "Baseline Viral Load",
      leu3n ~ "Baseline CD4+ Count",
      agg_phys ~ "Baseline Physical QoL",
      agg_ment ~ "Baseline Mental QoL"
    ),
    statistic = list(
      all_continuous() ~ "{mean} ({sd})",
      vload ~ "{median} ({p25}, {p75})" 
    ),
    missing_text = "(Missing)"
  ) %>%
  add_overall() %>%
  add_p(test = list(
    #continuous vars: welch two-sample t-test (default in t.test)
    all_continuous() ~ "t.test",
    #categorical vars: pearson chi-square test
    all_categorical() ~ "chisq.test",
    #skewed continuous var: wilcoxon rank sum test
    vload ~ "wilcox.test"
  )) %>% 
  modify_header(
    label ~ "**Variable**",
    all_stat_cols() ~ "**{level}** (N = {n})",
    stat_0 ~ "**Overall** (N = {N})"
  ) %>%
  bold_labels() %>%
  modify_fmt_fun(
    p.value = function(x) gtsummary::style_pvalue(x, digits = 3)
  )

#table name (separate from caption) + caption on bottom
table1 <- table1 %>%
  modify_spanning_header(
    all_stat_cols() ~ "**Table 1. Baseline Characteristics of Participants by Hard Drug Use Status**"
  )

#render as gt and add caption as a source note (bottom)
table1_gt <- table1 %>%
  as_gt() %>%
  gt::tab_source_note(
    gt::md("p-values from Welch two-sample t-tests for continuous variables, Pearson chi-squared tests for categorical variables, and Wilcoxon rank-sum test for viral load.")
  )

table1_gt
```

Section 4: Table 2

```{r}

#restrict to year 2 and create log10 viral load for model alignment
table2_data <- df_a %>%
  filter(years == 2) %>%
  distinct(newid, .keep_all = TRUE) %>%
  mutate(
    log10_vload = ifelse(!is.na(vload) & vload > 0, log10(vload), NA_real_)
  ) %>%
  select(hard_drugs_bl, adh_cat, vload, log10_vload, leu3n, agg_phys, agg_ment)

#create the table
table2 <- table2_data %>%
  tbl_summary(
    by = hard_drugs_bl,
    label = list(
      adh_cat ~ "Year 2 Adherence",
      vload ~ "Year 2 Viral Load",
      log10_vload ~ "Year 2 log10(VLOAD)",
      leu3n ~ "Year 2 CD4+ Count",
      agg_phys ~ "Year 2 Physical QoL",
      agg_ment ~ "Year 2 Mental QoL"
    ),
    statistic = list(
      adh_cat ~ "{n} ({p}%)",
      vload ~ "{median} ({p25}, {p75})",
      log10_vload ~ "{mean} ({sd})",
      leu3n ~ "{mean} ({sd})",
      agg_phys ~ "{mean} ({sd})",
      agg_ment ~ "{mean} ({sd})"
    ),
    missing_text = "(Missing)"
  ) %>%
  add_overall() %>%
  add_p(test = list(
    #continuous vars: welch two-sample t-test
    log10_vload ~ "t.test",
    leu3n ~ "t.test",
    agg_phys ~ "t.test",
    agg_ment ~ "t.test",
    #categorical vars: pearson chi-square test
    adh_cat ~ "chisq.test",
    #skewed continuous var: wilcoxon rank sum test
    vload ~ "wilcox.test"
  )) %>%
  modify_header(
    label ~ "**Variable**",
    all_stat_cols() ~ "**{level}** (N = {n})",
    stat_0 ~ "**Overall** (N = {N})"
  ) %>%
  bold_labels() %>%
  modify_fmt_fun(
    p.value = function(x) gtsummary::style_pvalue(x, digits = 3)
  )

#table name (separate from caption) + caption on bottom
table2 <- table2 %>%
  modify_spanning_header(
    all_stat_cols() ~ "**Table 2. Treatment Response Outcomes and Adherence at Year 2 by Baseline Hard Drug Use**"
  )

#render as gt and add caption as a source note (bottom)
table2_gt <- table2 %>%
  as_gt() %>%
  gt::tab_source_note(
    gt::md("p-values from Welch two-sample t-tests for continuous variables, Pearson chi-squared tests for categorical variables, and Wilcoxon rank-sum test for raw viral load.")
  )

table2_gt
```



Section 5: Initial exploration of outcome distributions
```{r}
#assess whether transformations of variables are needed
#restrict to year 2 data
df_y2 <- df_a %>%
  filter(years == 2)

# raw distributions
ggplot(df_y2, aes(x = vload)) + geom_histogram(bins = 60) + labs(title = "Year 2: VLOAD (raw)")
ggplot(df_y2, aes(x = leu3n)) + geom_histogram(bins = 60) + labs(title = "Year 2: LEU3N (raw)")
ggplot(df_y2, aes(x = agg_phys)) + geom_histogram(bins = 40) + labs(title = "Year 2: AGG_PHYS (raw)")
ggplot(df_y2, aes(x = agg_ment)) + geom_histogram(bins = 40) + labs(title = "Year 2: AGG_MENT (raw)")

#log transformations on vload and leu3n
ggplot(df_y2, aes(x = log10(vload))) + geom_histogram(bins = 60) + labs(title = "Year 2: log10(VLOAD)")
ggplot(df_y2, aes(x = log(leu3n))) + geom_histogram(bins = 60) + labs(title = "Year 2: log(LEU3N)")

```


```{r}
#year 2 dataset and transformations used for plots
df_y2 <- df_a %>%
  filter(years == 2) %>%
  mutate(
    log10_vload = ifelse(!is.na(vload), log10(vload), NA_real_)
  )

#summary statistics by exposure group (year 2)
y2_summary <- df_y2 %>%
  group_by(hard_drugs_bl) %>%
  summarise(
    n_subjects = n_distinct(newid),

    vload_med = median(vload, na.rm = TRUE),
    vload_p25 = quantile(vload, 0.25, na.rm = TRUE),
    vload_p75 = quantile(vload, 0.75, na.rm = TRUE),

    log10_vload_mean = mean(log10_vload, na.rm = TRUE),
    log10_vload_sd   = sd(log10_vload, na.rm = TRUE),
    log10_vload_med  = median(log10_vload, na.rm = TRUE),

    leu3n_mean = mean(leu3n, na.rm = TRUE),
    leu3n_sd   = sd(leu3n, na.rm = TRUE),
    leu3n_med  = median(leu3n, na.rm = TRUE),

    agg_phys_mean = mean(agg_phys, na.rm = TRUE),
    agg_phys_sd   = sd(agg_phys, na.rm = TRUE),
    agg_phys_med  = median(agg_phys, na.rm = TRUE),

    agg_ment_mean = mean(agg_ment, na.rm = TRUE),
    agg_ment_sd   = sd(agg_ment, na.rm = TRUE),
    agg_ment_med  = median(agg_ment, na.rm = TRUE)
  )

y2_summary

#boxplots for year 2 outcomes by baseline hard drug use
ggplot(df_y2, aes(x = hard_drugs_bl, y = log10_vload)) +
  geom_boxplot() +
  labs(title = "Year 2 Viral Load by Baseline Hard Drug Use",
       x = NULL, y = "log10(VLOAD)")

ggplot(df_y2, aes(x = hard_drugs_bl, y = leu3n)) +
  geom_boxplot() +
  labs(title = "Year 2 CD4 (LEU3N) by Baseline Hard Drug Use",
       x = NULL, y = "LEU3N")

ggplot(df_y2, aes(x = hard_drugs_bl, y = agg_phys)) +
  geom_boxplot() +
  labs(title = "Year 2 Physical QoL by Baseline Hard Drug Use",
       x = NULL, y = "AGG_PHYS")

ggplot(df_y2, aes(x = hard_drugs_bl, y = agg_ment)) +
  geom_boxplot() +
  labs(title = "Year 2 Mental QoL by Baseline Hard Drug Use",
       x = NULL, y = "AGG_MENT")

```



